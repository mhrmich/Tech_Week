...
function handleTempoSet(value: number): void {
  if (!_loaded || !_players) {
    console.warn("⚠️ Cannot set tempo - stems not loaded");
    return;
  }
  ...
  // Apply to all loaded stems
  if (_players.vocals.buffer.loaded) _players.vocals.playbackRate = clamped;
  if (_players.drums.buffer.loaded) _players.drums.playbackRate = clamped;
  if (_players.bass.buffer.loaded) _players.bass.playbackRate = clamped;
